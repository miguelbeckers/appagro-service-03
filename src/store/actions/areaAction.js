...
export const getAreasByUser = (id) => async dispatch => {
    try {
        dispatch({
            type: areaConstant.GET_AREA_LIST_LOADING
        });

        const res = await areaService.getAreasByUser(id);
...
export const deleteArea = (areaId, userId) => async dispatch => {
    try {
        dispatch({
            type: areaConstant.DELETE_AREA_LOADING
        });

        const res = await areaService.deleteArea(areaId, userId);
